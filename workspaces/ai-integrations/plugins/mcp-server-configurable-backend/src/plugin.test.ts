...
import { mcpServerConfigurablePlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [mcpServerConfigurablePlugin],
    });

    await request(server)
      .get('/api/mcp-server-configurable/todos')
      .expect(200, {
        items: [],
      });

    const createRes = await request(server)
      .post('/api/mcp-server-configurable/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get(`/api/mcp-server-configurable/todos/${createdTodoItem.id}`)
